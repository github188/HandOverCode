...
NET_DVR_Init();
	...
	m_JLLlist.InsertColumn(0,_T("编号"),LVCFMT_LEFT,55,-1);
	m_JLLlist.InsertColumn(1,_T("IP"),LVCFMT_LEFT,110,-1);
	m_JLLlist.InsertColumn(2,_T("用户"),LVCFMT_LEFT,100,-1);
	m_JLLlist.InsertColumn(3,_T("密码"),LVCFMT_LEFT,100,-1);
	m_JLLlist.InsertColumn(4,_T("端口"),LVCFMT_LEFT,60,-1);
	m_JLLlist.InsertColumn(5,_T("通道"),LVCFMT_LEFT,40,-1);
	////显示考车的列表
...
//	m_SecList.InsertColumn(1,_T("解码器"),LVCFMT_LEFT,120,-1);
	...
	//20160720 第二步 动态加载DLL
// 	dllHandleHKSS = LoadLibrary(TEXT("CVIDEO.dll"));
// 	if (dllHandleHKSS ==NULL)
// 	{
// 		AfxMessageBox("加载文件 CVIDEO.dll 失败!");
// 		CDialog::OnCancel();
// 		return FALSE;
// 	}
// 	
// 	dllHK_SHOWSTRING=(HK_SHOWSTRING) GetProcAddress(dllHandleHKSS,TEXT("HK_SHOWSTRING"));	
// 	if (dllHK_SHOWSTRING ==NULL )
// 	{
// 		AfxMessageBox("加载CVIDEO.dll 函数失败 !");
// 		CDialog::OnCancel();
// 		return FALSE;
// 	}
	//////////////////////////////////////////////////////////////////////////
...
BOOL CTest2Dlg::Oninittext()
{
	...
	for (i;i <= (int)inum;i++)
	{
		...
		temp2 = temp.Mid(pos+1);
		pos = temp2.Find(",");
		sPort = temp2.Left(pos);
		m_JLLlist.SetItemText(m_num,4,sPort);
		

		temp = temp2.Mid(pos+1);
		m_JLLlist.SetItemText(m_num,5,temp);
		m_num++;	
	}
...
//	temp6.Format("%s",strIP);
	...
	temp9.Format("是否要修改或添加： \n :%s\n IP:%s\n 用户名:%s\n 密码:%s\n 端口:%s\n\n若不变，看做修改！",m_num,temp6,m_user,m_pwd,m_port);
	if (MessageBox(temp9,"提示",MB_OKCANCEL) == IDOK)
	{	//修改
		temp1.Format("%s",m_num);
		temp.Format("%s,%s,%s,%s,%s",temp6,m_user,m_pwd,m_port,m_TD);
...
{
	// TODO: Add your control notification handler code here
	int item=m_JLLlist.GetItemCount();
	CString sIP,sPort,sUser,sPWD,sTd;
	for (int i=0;i<item;i++)
	{
		sIP = m_JLLlist.GetItemText(i,1);
		sUser = m_JLLlist.GetItemText(i,2);
		sPWD = m_JLLlist.GetItemText(i,3);
		sPort = m_JLLlist.GetItemText(i,4);
		sTd = m_JLLlist.GetItemText(i,5);
		TRACE("%s,%s,%s,%s,%s\n",sIP,sUser,sPWD,sPort,sTd);
		OnLoginHK(sIP,sUser,sPWD,atoi(sPort),atoi(sTd));
	}
}
...
// 	//20160720 释放 没释放也没有关系 反正程序都退出了。
// 	//////////////////////////////////////////////////////////////////////////
// 	if (dllHandleHKSS != NULL)
// 	{
// 		FreeLibrary(dllHandleHKSS);
// 	}
	//////////////////////////////////////////////////////////////////////////
	CDialog::OnClose();
}
...
void CTest2Dlg::WriteLog(LPCTSTR pstrFormat, ...)
{
	CString logstr;
...
BOOL CTest2Dlg::OnLoginHK(CString sIPaddr,CString sUser,CString sPwd,WORD wPort,int iChan)
{
	NET_DVR_DEVICEINFO_V30 struDeviceInfo;
	memset(&struDeviceInfo, 0, sizeof(NET_DVR_DEVICEINFO_V30));//存放设备参数的结构体
	LONG m_lLoginID=-1;
	m_lLoginID = NET_DVR_Login_V30((LPSTR)(LPCTSTR)sIPaddr,wPort,(LPSTR)(LPCTSTR)sUser,(LPSTR)(LPCTSTR)sPwd, &struDeviceInfo);
	if (m_lLoginID < 0)
	{
		WriteLog("登录视频服务器失败!Eid=%d,IP=%s,%s,%s",NET_DVR_GetLastError(),sIPaddr,sUser,sPwd);
		return FALSE;
	}
	WriteLog("登录成功!%s_%d",sIPaddr,wPort);
	DWORD dwReturned;
//	int m_iChanShowNum;
	NET_DVR_SHOWSTRING_V30 m_struShowString;
	memset(&m_struShowString, 0, sizeof(m_struShowString));
	if (!NET_DVR_GetDVRConfig(m_lLoginID, NET_DVR_GET_SHOWSTRING_V30, iChan, &m_struShowString, sizeof(NET_DVR_SHOWSTRING_V30), &dwReturned))
	{
	...
	{
		temp.Format("%d",i);
		ictd=GetPrivateProfileInt("OSDLINE",temp,1,".\\config.ini");
		if (ictd==1)
		{
			if (m_struShowString.struStringInfo[i].wShowString==1)
			{
				m_struShowString.struStringInfo[i].wShowString=0;
				WriteLog("设置不显示sIPaddr=%s,iChan=%d,行=%d",sIPaddr,iChan,i);
			}
		}
		
	}

	if (!NET_DVR_SetDVRConfig(m_lLoginID, NET_DVR_SET_SHOWSTRING_V30,iChan, &m_struShowString, sizeof(NET_DVR_SHOWSTRING_V30)))
	{
			WriteLog("设置字符参数失败!ErrorID=%d",NET_DVR_GetLastError());
			return FALSE;
	}
	WriteLog("设置字符参数成功!%s,%d",sIPaddr,iChan);
	return TRUE;
}
