...
BEGIN_MESSAGE_MAP(CDlgPlayWnd, CDialog)
	...
	ON_COMMAND(IDM_MENU_RGPP, OnMenuRgpp)
	ON_WM_RBUTTONDOWN()
...
void CDlgPlayWnd::OnEPlaywnd2(CString errocode)
{
	...
	if (playwnd2.DvrInitLogin(hwnd,m_xmsbip.GetBuffer(0),m_xmsbuser.GetBuffer(0),m_xmsbpwd.GetBuffer(0),atoi(m_xmsbport)))
	{
		...
		logz.WriteLog(logxmid,"登录设备成功!%s,%s_%s_%s_%s_s",errocode,m_xmsbip,m_xmsbuser,m_xmsbpwd,m_xmsbport,m_xmsbtd);
		playwnd2.ISPlaywndd=TRUE;
	}	
	else
	{
		logz.WriteLog(logxmid,"登录失败!设备%s,%s_%s_%s_%s_s",errocode,m_xmsbip,m_xmsbuser,m_xmsbpwd,m_xmsbport,m_xmsbtd);
	}
}
...
void CDlgPlayWnd::OnMenuRgpp() 
{
	// TODO: Add your command handler code here
	CString strKchp,strStuName,strStuZkzm;	
	strKchp.Format("考车%d",uKch);
//	AfxMessageBox(str);
	CString temp;
	//1.根据考车号读取考试中考生
	try
...
// 			_RecordsetPtr pSet1 =m_pConnwnd->Execute((_bstr_t)temp1,&cnt1,adCmdUnknown);
// 			_variant_t vat1;
// 			
...
void CDlgPlayWnd::OnRButtonDown(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	if (uKch!=0)
	{		
