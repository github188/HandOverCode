...
BEGIN_MESSAGE_MAP(CPLAYWND, CDialog)
	...
	ON_COMMAND(IDM_RGPPDLL, OnRgppdll)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CPLAYWND::OnMsoundshare() 
{
	...
	{
		//zsz 20161101  
		//::PostMessage(AfxGetMainWnd()->m_hWnd,WM_STARTALLVIDEO_MSG,1,0);
		if (m_bSwitch)
		{
...
void CPLAYWND::SetPageCarNum(int ich)
{
	CString temp;
	temp.Format("画面%d",ich);
	iKch=GetPrivateProfileInt("画面",temp,0,".\\aconfig.ini");
	temp.Format("考车%d",iKch);
	GetDlgItem(IDC_STATIC_MSG)->SetWindowText(temp);
	...
	if (m_bPlaySound)
	{
		if (m_bSwitch)
		{
			NET_DVR_CloseSoundShare(m_lPlayHandle[1]);
		}
		else
		{
			NET_DVR_CloseSoundShare(m_lPlayHandle[2]);
		}
	}

	for (int i=0;i<3;i++)
	{
		if (m_lVoiceHandle[i]!=-1)
		{
			NET_DVR_StopVoiceCom(m_lVoiceHandle[i]);
			m_lVoiceHandle[i]=-1;
		}		
		if (m_lPlayHandle[i]!=-1)
		{
		...
		if (m_lUserID[i]!=-1)
		{
			NET_DVR_Logout_V30(m_lUserID[i]);
			m_lUserID[i]=-1;
		}		
		m_lChannel[i]=-1;
		m_byStartDTalkChan[i]=-1;
...
void CPLAYWND::SetPlaySound()
{
	m_bPlaySound=false;
}

void CPLAYWND::OnRgppdll() 
{
	// TODO: Add your command handler code here
	::PostMessage(AfxGetMainWnd()->m_hWnd,WM_RGPPDLL_MSG,iKch,0);
}
