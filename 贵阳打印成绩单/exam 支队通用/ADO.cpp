...
void ADO::OnInitADOConn(CString m_IP,CString m_DataBase,CString m_Name,CString m_Pwd,int SQLorOracle)
{
	CString m_strInEdit;
	::CoInitialize(NULL);
	try
	{
	   ...
	   */
	   CString strConnect;
		if (SQLorOracle==1)
		{
			m_strInEdit.Format("driver={SQL Server};Server=%s;DATABASE=%s;UID=%s;PWD=%s",m_IP,m_DataBase,m_Name,m_Pwd);
		}else
		{
			m_strInEdit.Format("Provider=OraOLEDB.Oracle.1;Data Source=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=%s)(PORT=1521))(CONNECT_DATA=(SERVICE_NAME=bkzy)));User ID=sapho;Password=%s",m_IP,m_Pwd);
		}
	...
	catch(_com_error e)
	{
		...
		errormessage.Format("数据库提示,连接照片数据库失败!,错误信息:%s,请先配置照片数据库连接,%s",e.ErrorMessage(),m_strInEdit);
