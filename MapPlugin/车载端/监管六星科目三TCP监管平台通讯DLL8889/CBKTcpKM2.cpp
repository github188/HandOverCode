...
DWORD WINAPI SendThreadProc(LPVOID param)
{
	...
	theApp.WriteLogOne("发送线程关闭!SendThreadProc Close!");
	return 0;
}
...
//	theApp.WriteLogOne("OnReceive(%d,%s)",CurSock,temp);//201705312048 加日志
	if (1 ==temp.Replace("*","*"))
	{
		CString tmp1,tmp2;
		...
		{
			theApp.WriteLogThree("20170531 2104 17Z02收到[%s]",temp);//201705312048 加日志
...
BOOL CCBKTcpKM2App::SetSysTime(CString strTime)
{
	...
	{		
		WriteLogOne("设置本机时间为:%s 失败!",strTime);
		return FALSE;
	}	
	WriteLogOne("设置本机时间为:%s 成功!",strTime);
	return TRUE;
}
...
void CCBKTcpKM2App::Send17Z02(CString str)
{
	...
	if(SOCKET_ERROR==WSASendTo(m_socketz,&wsabuf,1,&dwSend,0,
		(SOCKADDR*)&addrSrv,sizeof(SOCKADDR),NULL,NULL))
	{
		WriteLogOne("发送的数据失败!%s",msendbuf);		
	}
	else
	{
		WriteLogThree("发送:%s",msendbuf);
	}
}