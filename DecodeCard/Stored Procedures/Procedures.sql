...
AND e.考试次数=s.考试次数 
AND e.当日次数=s.当日次数 
AND e.准考证明编号=[dbo].charEncode(@ZKZM_Number)
ORDER BY 记录编号 ASC
...
SELECT @Status=状态,@KscsN=考试次数,@DrcsN=当日次数 FROM Studentinfo WHERE 准考证明编号=[dbo].charEncode(@ZKZM_Number)
SELECT @KSCJ=100-SUM([dbo].intDecode(扣除分数)) FROM ErrorRecords JOIN ErrorData ON  ErrorRecords.错误编号=ErrorData.错误编号 
WHERE ErrorRecords.准考证明编号=[dbo].charEncode(@ZKZM_Number) and 考试次数=@KscsN and 当日次数=@DrcsN

set @intErrorCode = 0
if exists(Select TOP 1 错误编号 from ErrorRecords where 准考证明编号=[dbo].charEncode(@ZKZM_Number) and 记录编号>@Ejlbh and 当日次数=@DrcsN and 考试次数=@KscsN)
begin --1b
Select TOP 1 @ErrorCode=错误编号,@ErrorCodejl=记录编号 from ErrorRecords where 准考证明编号=[dbo].charEncode(@ZKZM_Number) and 记录编号>@Ejlbh and 当日次数=@DrcsN and 考试次数=@KscsN
...
WHERE er.准考证明编号=[dbo].charEncode(@ZKZM_Number) and 考试次数=@KscsN and 当日次数=@DrcsN and ed.扣除分数!=[dbo].intEncode(0, RAND()) and er.记录编号<=@Ejlbh
