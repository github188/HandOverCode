...
SELECT 准考证明编号,考车号,姓名 as MSG0,性别 as MSG1,流水号 as MSG2,身份证明编号 as MSG3,驾校名称 as MSG4,考试员1 as MSG5,当日次数 as MSG6,考车号+'-'+SysCfg.备注+'-'+考试车型 as MSG7
FROM StudentInfo 
LEFT JOIN SchoolInfo ON 代理人=驾校编号 
JOIN SysCfg ON 考车号=项目
WHERE 登录状态='已登录' 
AND StudentInfo.状态='3' 
AND (Select CONVERT(varchar(100), 预考日期, 23)) = (Select CONVERT(varchar(100), GETDATE(), 23))
/*--2014-07-07 ZSZ*/
...
SET NOCOUNT ON --不返回计数
Set ARITHABORT ON
SELECT e.记录编号,e.错误编号,CONVERT(varchar(10),e.出错时间,8) as 出错时间,ed.扣分类型 as 扣分类型,ed.扣除分数 as 扣除分数 FROM ErrorRecords e,StudentInfo s,ErrorData ed
...
if exists(select * from errordata where 扣除分数!=0 and 错误编号=@ErrorCode) 
BEGIN --2b
declare @counterrline int --统计错误行
SELECT @counterrline=Count(*) FROM ErrorRecords er JOIN ErrorData ed ON  er.错误编号=ed.错误编号 
WHERE er.准考证明编号=@ZKZM_Number and 考试次数=@KscsN and 当日次数=@DrcsN and ed.扣除分数!=0 and er.记录编号<=@Ejlbh

Select er.记录编号,ed.扣分类型 as 扣分类型,CONVERT(varchar(10),er.出错时间,8) as 出错时间,ed.扣除分数 as 扣除分数,成绩=@KSCJ,ELine=@counterrline
...
begin --5b
Select er.记录编号,ed.错误编号,ed.扣分类型
from ErrorRecords er JOIN ErrorData ed ON er.错误编号=ed.错误编号 
where er.记录编号=@ErrorCodejl
set @intErrorCode = 2
goto ERR_HANDLER
