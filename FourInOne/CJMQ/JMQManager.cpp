...
{
	AppLogID=logz.Register("解码器主程序.log");
	dwSLogID=logz.Register("JMQSuccess.log");
...
{	
	...
	int jmqnum=GetPrivateProfileInt("JMQ","NUM",0,configfile);//读取解码器个数
	int EVEN=GetPrivateProfileInt("JMQ","EVEN",0,configfile);//读取是否隔BNC解码
	uKSKM=GetPrivateProfileInt("CONFIG","KSKM",2,configfile);//考试科目
	uWNDTWO=GetPrivateProfileInt("CONFIG","WND2",1,configfile);//画面二状态 0
	logz.WriteLog(AppLogID,"读取到%d台解码器!EVEN=[%d],科目%d,画面2=%d,HCNetSDK V%d.%d.%d.%d",jmqnum,EVEN,uKSKM,uWNDTWO,\
	...
	for (i=1;i<=jmqnum;i++)
	{
		if (OnDS63Init(i)==true)//设备初始化成功
		{
			...
			for (int j=0;j<m_Ability.struBncInfo.byChanNums;j++)//BNC个数循环
			{
				if (EVEN==1 && j%2==1)
				{
					continue;
				...
				if (0 == ukch)//没有配置
				{
					logz.WriteLog(AppLogID,"解码器[JMQ%d]的BNC通道[%s]处于空闲,可以配置!",i,lpKeyName);
				}
				else
				{
					OnCheckChan(ukch,j);
		...
		{
			logz.WriteLog(AppLogID,"解码器[JMQ%d]未初始化成功!",i);
		}
	}
}
...
bool CJMQManager::OnDS63Init(int id)
{
	...
	logz.WriteLog(AppLogID,"解码器=[%d],IP=%s,[%s],DecN=%d,BncN=%d",id,sIPaddr,cTemp,m_Ability.byDecChanNums,m_Ability.struBncInfo.byChanNums);
	//////////////////////////////////////////////////////////////////////////
...
bool CJMQManager::OnCheckChan(UINT kch,int bncn)//考车号 BNC编号
{
	...
	dwDispChan=m_Ability.struBncInfo.byStartChan+bncn;//显示通道
	//解码器获取显示通道配置
		...
		(m_DispChanCfg.byJoinDecChan[3] != byDecChan[m_DisPlayShowi[3]]) )//显示通道不是四分割输出
	{
		...
		m_DispChanCfg.byVedioFormat =2;//视频制式：1- NTSC，2- PAL
		m_DispChanCfg.dwResolution= 0;
		...
		if (!NET_DVR_MatrixSetDisplayCfg_V41(m_lUserID,dwDispChan,&m_DispChanCfg)) 
        {
			...
			{
				logz.WriteLog(dwFLogID,"错误29:设备操作失败,请将解码器完全恢复下.设备管理=>恢复默认参数=>完成恢复");
			}
...
//	m_JMQCar[ikch].On17C51(str,ikscs,idrcs);
	...
	logz.WriteLog(dwSLogID,"OnJ17C51(%d,%s)",ikch,str);
	//////////////////////////////////////////////////////////////////////////
// 	CString temp;
// 	STerminalPZ temppz;
// 	temp.Format("%s_1","10001");//201	
// 	if(GetTerminalPZ(temppz,temp)==TRUE)
// 	{
			...
			if (OnFindCar(ikch))
		 	{
				...
				case 6:
					...
					break;
				default:
					logz.WriteLog(AppLogID,"default icmd=%d",icmd);
					break;
				}
...
DWORD WINAPI QHThreadFuncc(LPVOID param)
{
	...
	CString temp,temp1;
	GetPrivateProfileString(errcode,"Time","2000",temp.GetBuffer(MAX_PATH),MAX_PATH,".\\config.ini");
	temp1.Format("%s",temp);
