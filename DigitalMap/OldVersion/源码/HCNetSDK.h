...
//******操作设备命令 begin****//
#define  NET_DVR_SECURE_PULL_RAID_DISK          6015  //安全拔出Raid中的硬盘
#define  NET_DVR_SCAN_RAID                      6016  //扫描阵列
//******操作设备命令 end****//
...
typedef struct 
{
	...
	BYTE byResolution;  	//分辨率0-DCIF 1-CIF, 2-QCIF, 3-4CIF, 4-2CIF 5（保留）,6-QVGA(320*240), 
							...
							//33-2448*800,34-XGA（1024*768），35-SXGA（1280*1024）,36-WD1(960*576/960*480),37-1080i	
							//0xff-Auto(使用当前码流分辨率)
	...
	// 13-384K 14-448K 15-512K 16-640K 17-768K 18-896K 19-1024K 20-1280K 21-1536K 22-1792K 23-2048K  24-2560K 25-3072K 26-4096K 27-5120K 28-6144K 29-7168K 30-8192K 31-16384k
...
typedef struct tagNET_DVR_EASYDDNS_STREAM
{
    BYTE    byEnable;				 // 是否在线
    BYTE    byRes[3];               // 保留字节
    BYTE    byEasyDDNSDomain[64];   // EasyDDNS服务器
    WORD    wPort;                  // EasyDDNS 端口
    WORD    wAliasLen;              // 别名长度
    BYTE    wAlias[NAME_LEN];		 // 别名
    WORD    wDVRSerialLen;          // 序列号长度
    BYTE    byRes1[2];              // 保留字节
    ...
    BYTE    byRes2[11];             // 保留字	
}NET_DVR_EASYDDNS_STREAM, *LPNET_DVR_EASYDDNS_STREAM;
...
typedef union tagNET_DVR_GET_STREAM_UNION
{
	...
	NET_DVR_EASYDDNS_STREAM struEasyDDNSStream;   //通过EasyDDNS去取流
...
{
    BYTE byEnableDDNS;
    BYTE byHostIndex;/*0-IPServer 1－Dyndns 2－PeanutHull(花生壳)，3- NO-IP, 4- EasyDDNS*/
    BYTE byRes1[2];
    struct
    {    
...
typedef struct tagNET_DVR_PLATE_RESULT
{
    ...
    BYTE    byRes2;                      //保留
    DWORD   dwBinPicLen; //二值图长度
	DWORD   dwCarPicLen;//近景图长度
    ...
    BYTE	*pBuffer1;   		        // 当上传的是图片，指针指向图片信息，当上传的是视频，指针指向视频信息，如果不想获得图片或视频信息，传NULL(DVS车辆近景图)
    BYTE    *pBuffer2;                  // 当上传的是图片时，指向车牌图片的指针（DVS车牌彩图）
...
NET_DVR_API BOOL __stdcall NET_DVR_DecodeG726Frame(void *pDecMoudle, unsigned char* pInBuffer, unsigned char* pOutBuffer, BYTE byReset);
//编码
NET_DVR_API void* __stdcall NET_DVR_InitG726Encoder(void** pEncMoudle);
NET_DVR_API BOOL __stdcall NET_DVR_EncodeG726Frame(void *pEncMoudle,unsigned char* pInBuffer, unsigned char* pOutBuffer, BYTE byReset);
...
NET_DVR_API BOOL CALLBACK NET_DVR_PreviewRequest(LONG lUserID, LONG lChannel, LPNET_DVR_PREVIEWPARAM lpPreviewParam, LPNET_DVR_DEVICENATINFO lpOuter);
...
typedef struct tagNET_DVR_DIALPARAM
{
    ...
    BYTE byUimNumber[MAX_PHONE_NUM/*32*/];
    BYTE byRes2[24];
...
参  数：
		...
		[in]byMode:监听模式，0-TCP监听，1-UDP监听，UDP监听用于设备查询其外网地址
...
{-11*3600,GMT-11:00}
{-10*3600,GMT-10:00}
{-9*3600-1800,GMT-09:30}
{-9*3600,GMT-09:00}
{-8*3600,GMT-08:00}
{-7*3600,GMT-07:00}
{-6*3600,GMT-06:00}
{-5*3600,GMT-05:00}
{-4*3600-1800,GMT-04:30}
{-4*3600,GMT-04:00}
{-3*3600-1800,GMT-03:30}
{-3*3600,GMT-03:00}
{-2*3600,GMT-02:00}
{-3600,GMT-01:00}
{0,GMT}
...
{4*3600 + 1800, GMT+04:30},
{5*3600, GMT+05:00},
{5*3600 + 1800, GMT+05:30},
{5*3600 + 2700, GMT+05:45},
{6*3600, GMT+06:00},
{6*3600 + 1800, GMT+06:30},
{7*3600, GMT+07:00},
{8*3600, GMT+08:00},
{9*3600, GMT+09:00},
{9*3600 + 1800, GMT+09:30},
{10*3600, GMT+10:00},
{10*3600 + 1800, GMT+10:30},
{11*3600, GMT+11:00},
{11*3600 + 1800, GMT+11:30},
{12*3600, GMT+12:00},
{13*3600, GMT+13:00}*/
...
typedef struct tagNET_DVR_ALARMHOST_ABILITY
{
    ...
    BYTE    bySupportAlarmInType[16];//支持的防区类型,数组0:即时防区,1-24小时防区,2-延时防区 ,3-内部防区，4-钥匙防区
...
typedef struct tagNET_DVR_SWITCH_ALARM
{
    ...
    BYTE	byAlarmType;		// 报警类型,0-正常，1-短路，2-断路
    BYTE 	byRes[41];			// 保留字节
...
typedef struct tagNET_DVR_ALARMHOSTDIALCFG
{
	...
	WORD wReportPeriod;      //测试报告上传周期，小时，（最大值支持7天，即168小时)
...
输入:	lUserID NET_DVR_Login返回值 
        lAlarmOutPort 报警输出口索引 从0开始  0xffffffff表示全部报警输出口
...
typedef struct tagNET_DVR_EVENT_CAPTURE
{
    ...
    NET_DVR_REL_CAPTURE_CHAN struRelCaptureChan[MAX_PIC_EVENT_NUM]; // 数组下标 0 移动侦测触发抓图 1 视频遮挡触发抓图
...
typedef struct tagNET_DVR_VIDEO_EFFECT
{
	...
	BYTE  byRes[20];          //保留
}NET_DVR_VIDEO_EFFECT, *LNET_DVR_VIDEO_EFFECT;

typedef struct tagNET_DVR_VIDEO_INPUT_EFFECT
{	
	DWORD  dwSize;           //结构体大小
	WORD  wEffectMode;         //模式,0:标准 1:预置模式1 2:预置模式2 3:预置模式3 255:自定义
	BYTE    byRes1[146];        //保留
	NET_DVR_VIDEO_EFFECT struCustomizeMode; //自定义模式参数
	BYTE   byRes2[60];        //保留
...
typedef struct tagNET_DVR_TIME_LOCK
{
	...
	DWORD   dwRecordType;     //录像类型:  0xff－全部，0－定时录像，1-移动侦测，2－报警触发，3-报警触发或移动侦测，4-报警触发和移动侦测，5-命令触发，6-手动录像，7-智能录像(同文件查找)
	DWORD   dwLockDuration;   //锁定持续时间,单位秒
	BYTE    byRes[12];
...
参数说明：
lUserID：NET_DVR_Login()的返回值
lpCamListInfo：LPNET_DVR_MATRIX_CAMERALIST结构指针
返回值： 
TRUE：成功，FALSE：失败   
***************************************************************/
...
参数说明：
lUserID：NET_DVR_Login()的返回值
dwCameraId：全局CAMERA编号
lpInter：LPNET_MATRIX_CAMERAINFO结构指针
返回值： 
TRUE：成功，FALSE：失败  
***************************************************************/
...
参数说明：
lUserID：NET_DVR_Login()的返回值
dwCameraNum：要添加的Camera数量
pBuffer：Camera信息缓冲区，dwCameraNum 个NET_MATRIX_CAMERAINFO结构长度
dwBufLength：缓冲区长度
返回值： 
TRUE：成功，FALSE：失败 
***************************************************************/
...
参数说明：
lUserID：NET_DVR_Login()的返回值
lpMonListInfo：LPNET_DVR_MATRIX_MONITORLIST结构指针
返回值： 
TRUE：成功，FALSE：失败 
***************************************************************/
...
参数说明：
lUserID：NET_DVR_Login()的返回值
dwMonitorId：全局MOTINOR编号
lpInter：LPNET_MATRIX_MONITORINFO结构指针
返回值： 
TRUE：成功，FALSE：失败  
***************************************************************/
...
参数说明：
lUserID：NET_DVR_Login()的返回值
dwMonitorNum：要添加的Monitor数量
pBuffer：MONITOR信息缓冲区
dwBufLength:缓冲区长度，dwMonitorNum个NET_MATRIX_MONITORINFO长度
返回值： 
			TRUE：成功，FALSE：失败 
***************************************************************/
...
参数说明：
lUserID：NET_DVR_Login()的返回值
lpMatrixListInfo：LPNET_DVR_MATRIXLIST结构指针
返回值： 
TRUE：成功，FALSE：失败     
***************************************************************/
...
lUserID：NET_DVR_Login()的返回值
dwMatrixId：MATRIX编号
lpInter：LPNET_MATRIX_MATRIXINFO结构指针
返回值： 
TRUE：成功，FALSE：失败    
***************************************************************/
...
lUserID：NET_DVR_Login()的返回值
dwSerialChan：串口号
lpInter：指向NET_MATRIX_UARTPARAM结构的指针
返回值： 
TRUE：成功，FALSE：失败  
***************************************************************/
...
lUserID：NET_DVR_Login()的返回值
dwUserNum：用户编号
lpInter：LPNET_MATRIX_USERPARAM结构指针
返回值： 
TRUE：成功，FALSE：失败   
***************************************************************/
...
lUserID：NET_DVR_Login()的返回值
dwResourceNum：资源组编号
lpInter：LPNET_MATRIX_RESOURSEGROUPPARAM结构指针
返回值： 
TRUE：成功，FALSE：失败   
***************************************************************/
...
参数说明：
lUserID：NET_DVR_Login()的返回值
dwUserGroupNum：用户组编号
lpInter：LPNET_MATRIX_USERGROUPPARAM结构指针
返回值： 
TRUE：成功，FALSE：失败     
***************************************************************/
...
/*说明：使用用户组的概念可以对用户进行权限管理和资源管理。因为用户组包含用户成员信息，管理资源信息，权限信息，所以, 可以实现用户的级别管理和权限管理*/
...
参数说明：
lUserID：NET_DVR_Login()的返回值
lpTrunkListInfo：LPNET_DVR_MATRIX_TRUNKLIST结构指针
返回值： 
TRUE：成功，FALSE：失败  
***************************************************************/
...
lUserID：NET_DVR_Login()的返回值
dwTrunkNum：干线编号
lpInter：LPNET_MATRIX_TRUNKPARAM结构指针
返回值： 
TRUE：成功，FALSE：失败 
***************************************************************/
...
lUserID：NET_DVR_Login()的返回值
dwTrunkNum：干线编号
byCtrlMode：控制方式，1-删除干线，2-预留干线，3-释放干线
返回值： 
TRUE：成功，FALSE：失败  
***************************************************************/
...
lUserID：NET_DVR_Login()的返回值
dwTrunkNum：干线编号
byTrunkStatus：干线状态，1-占用，2-空闲，3-预留
返回值： 
TRUE：成功，FALSE：失败   
***************************************************************/
...
//向设备发送命令，做操作：安全拔盘，重新扫描等等。
NET_DVR_API BOOL __stdcall NET_DVR_CommandDevice(LONG lUserID, DWORD dwCommand, LPVOID lpInBuffer, DWORD dwInLen);
...
// NET_DVR_API LONG __stdcall NET_DVR_AdvanceFindDBInfo(LONG lUserID, LPNET_VCA_FIND_PICTURECOND_ADVANCE lpFindParam);
// NET_DVR_API LONG __stdcall NET_DVR_FindNextDBInfo(LONG lFindHandle, LPNET_VCA_FINDDB_RET lpFindDBRet);
// NET_DVR_API BOOL __stdcall NET_DVR_FindDBInfoClose(LONG lFindHandle);
...
/**************************************************************
函数名：NET_DVR_MatrixGetSceneCfg
功能：获取场景配置
参数说明：
lUserID[in]：NET_DVR_Login()的返回值
dwSceneNum[in]：场景号
lpSceneCfg[out]：指向NET_MATRIX_SCENECFG结构指针
返回值： 
TRUE：成功，FALSE：失败   
***************************************************************/
...
/**************************************************************
函数名：NET_DVR_MatrixSetSceneCfg
功能：配置场景模式参数
参数说明：
lUserID[in]：NET_DVR_Login()的返回值
dwSceneNum[in]：场景号
lpSceneCfg[in]：指向NET_MATRIX_SCENECFG结构指针
返回值： 
TRUE：成功，FALSE：失败   
***************************************************************/
...
/**************************************************************
函数名：NET_DVR_MatrixSceneControl
功能：场景切换控制
参数说明：
lUserID[in]：NET_DVR_Login()的返回值
dwSceneNum[in]：场景号
dwCmd:命令类型，1-场景模式切换，2-初始化场景（将此场景的配置清空，如果是当前场景，则同时对当前场景进行清屏操作），3-强制切换（此种模式可以支持向当前场景切换）
dwCmdParam:命令参数
返回值： 
TRUE：成功，FALSE：失败   
***************************************************************/
...
/**************************************************************
函数名：NET_DVR_MatrixGetCurrentSceneMode
功能：获取当前正在使用的场景模式
参数说明：
lUserID[in]：NET_DVR_Login()的返回值
dwSceneNum：场景号,输出参数
...
typedef struct tagNET_DVR_SCREEN_WINCFG
{
	DWORD	dwSize;
	BYTE	byVaild;
	BYTE	byInputType;		//输入源类型,0-video,1-RGB,2-网络
...
/**************************************************************
函数：NET_DVR_GetAllValidWinInfo
功能:获取所有有效窗口信息
参数说明:
lUserID: NET_DVR_Login()的返回值
lpWinListInfo:指向NET_DVR_WINLIST结构的指针
TRUE表示成功,FALSE表示失败
***************************************************************/
NET_DVR_API BOOL __stdcall NET_DVR_GetAllValidWinInfo(LONG lUserID, LPNET_DVR_WINLIST lpWinListInfo);
/**************************************************************
函数：NET_DVR_ScreenWinCtrl
功能:窗口控制
参数说明:
lUserID: NET_DVR_Login()的返回值
dwWinIndex：窗口序号，对于大屏服务器，高两字节表示设备号，低两字节表示窗口号；对于海康大屏控制器，表示窗口号
dwCommand:命令值1-open，2-close（窗口号0xffff表示对所有窗口的操作）3-top(窗口置顶)
lpInter：NET_DVR_WINCFG结构指针，命令值为open时传入的参数
返回值：TRUE表示成功,FALSE表示失败
***************************************************************/
...
typedef struct _NET_DVR_INPUTSTREAMCFG
{
	DWORD	dwSize ;
	BYTE	byValid ;
	BYTE	byCamMode;						//信号输入源类型，见CAM_MODE
...
typedef struct tagNET_DVR_SCREENCONTROL_ABILITY
{
	...
	BYTE byInputStreamNum;  /*输入源个数*/
	BYTE byOutputChanNum;	/*输出通道个数*/
    ...
    BYTE byMatrixInputNum;  //输入源支持最大个数
    BYTE byMatrixOutputNum; //输出源支持最大个数
...
{
	WORD	wInputNo;		/*信号源序号*/
	BYTE	byInputType;	//1：CAM_BNC  2：CAM_VGA  3: CAM_DVI  4：CAM_HDMI  5：CAM_IP
	BYTE	byRes1[9];
...
/*************************************************
函数：NET_DVR_GetScreenInputStatus
功能：获取输入信号状态
参数说明：
lpStatus：输入信号状态结构体
返回值：TRUE：成功，FALSE：失败 
...
{
	DWORD	dwSize;
    BYTE	byAlarmType;	//报警类型，1-子板拔出，2-子板插入，3-子系统状态异常，4-子系统恢复恢复 5-输入源异常   6-温度报警
	BYTE	byBoardType;	// 1-输入板 2-输出板 
	BYTE	bySubException;	//输入异常时具体子异常（其它目前build还没有） 1- 分辨率改变2-接口改变3-分辨率错误
	BYTE	byRes1; 
...
NET_DVR_API BOOL __stdcall NET_DVR_RemoteControlAlarm(LONG lUserID, LONG lCommand);
NET_DVR_API BOOL __stdcall NET_DVR_RemoteControlStudy(LONG lUserID);
...
{
    DWORD    dwSize;					//结构体大小
    BYTE     byPicQuota;				//图片百分比
	BYTE     byRecordQuota;			//录像百分比
	BYTE     byDrawFrameRecordQuota;	//抽帧录像百分比
    BYTE     byRes[61];				//保留字节
...
typedef struct  tagNET_DVR_UPNP_CFG
{
    ...
    NET_DVR_UPNP_PORT    struWebPort;   //web server端口映射配置
    NET_DVR_UPNP_PORT    struAdminPort; //管理端口映射配置
...
typedef struct
{
	...
	WORD  wMainProType;        //主码流协议类型 0 RTP 1 http
	WORD  wMainPort;           //主码流端口	
	char  sMainPath[MAX_PRO_PATH];  //主码流路径
	
	WORD  wSubProType;         //子码流协议类型 0 RTP 1 http
	WORD  wSubPort;            //子码流端口
...
NET_DVR_API LONG __stdcall NET_DVR_PlayBackReverseByName(LONG lUserID, char *sPlayBackFileName, HWND hWnd);
NET_DVR_API LONG __stdcall NET_DVR_PlayBackByTime_V40(LONG lUserID, HWND hWnd, LPNET_DVR_PLAYCOND  pPlayCond);
...
{
    DWORD   dwSize;
    WORD    wEnterDelay;//进入延时,单位:秒
    WORD	wExitDelay;//退出延时,单位:秒
...
{ 
	...
	BYTE	byReportProtocol;		//1-hik 2-NAL2300
...
NET_DVR_API BOOL __stdcall NET_DVR_SetStreamConfig(LONG nUserID, DWORD nCommand, DWORD nIDCount, LPVOID lpIDBuffer, DWORD nIDBufferSize, LPVOID lpInBuffer, DWORD nInBufferSize);

NET_DVR_API BOOL __stdcall NET_DVR_GetStreamConfig(LONG nUserID, DWORD nCommand, DWORD nIDCount, LPVOID lpInBuffer, DWORD nInBufferSize, LPVOID lpOutBuffer, DWORD nOutBufferSize, DWORD *lpBytesReturned);
...
NET_DVR_API int __stdcall NET_DVR_PlayBack(LONG lUserID, LPNET_DVR_VOD_PARA pVodPara);
